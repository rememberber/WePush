...
WePush Next `0.1.0-alpha.4` 是面向社区的公开预览版，用于验证新架构、收集反馈和开展兼容性测试；它不是稳定版，也不建议直接用于关键生产业务。
...
- `alpha.4` 的 SMTP、企业 IM、短信和微信 Provider 会在正式 Run 中调用用户配置的第三方账号，并可能消耗渠道额度或产生第三方费用。务必先用自有测试目标完成 Dry Run 和小范围验证。
