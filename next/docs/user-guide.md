...
- [WePush Next `0.1.0-alpha.1` Public Preview](https://github.com/rememberber/WePush/releases/tag/next-v0.1.0-alpha.1)

发行附件：

| 文件 | 内容或平台 |
| --- | --- |
| `wepush-next-0.1.0-alpha.1.tar.gz` | Service、Agent、WebUI、安装脚本、OpenAPI 与 SDK，适合 Linux/macOS |
| `wepush-next-0.1.0-alpha.1.zip` | 与上面内容相同，适合 Windows |
...
```bash
sha256sum wepush-next-0.1.0-alpha.1.tar.gz
grep 'wepush-next-0.1.0-alpha.1.tar.gz$' SHA256SUMS
```

macOS：

```bash
shasum -a 256 wepush-next-0.1.0-alpha.1.tar.gz
grep 'wepush-next-0.1.0-alpha.1.tar.gz$' SHA256SUMS
```
...
```powershell
Get-FileHash .\wepush-next-0.1.0-alpha.1.zip -Algorithm SHA256
Select-String -Path .\SHA256SUMS -Pattern 'wepush-next-0.1.0-alpha.1.zip$'
```
...
```bash
tar -xzf wepush-next-0.1.0-alpha.1.tar.gz
cd wepush-next-0.1.0-alpha.1
./bin/wepush-service
```
...
```powershell
Expand-Archive .\wepush-next-0.1.0-alpha.1.zip .\wepush-next
Set-Location .\wepush-next\wepush-next-0.1.0-alpha.1
...
- Linux：运行 `wepush-next-desktop-0.1.0-alpha.1-linux-x64/WePush Next/wepush-next`。
- macOS：打开 `wepush-next-desktop-0.1.0-alpha.1-macos-arm64/WePush Next.app`。
- Windows：运行 `wepush-next-desktop-0.1.0-alpha.1-windows-x64\WePush Next\WePush Next.exe`。
...
当前 `0.1.0-alpha.1` WebUI 的任务卡片只提供 Dry Run。正式运行可通过“API 文档”页中的 `createRun` 操作、Java SDK 或直接调用 Service API 发起。
...
```bash
tar -xzf wepush-next-0.1.0-alpha.1.tar.gz
sudo ./wepush-next-0.1.0-alpha.1/install/linux/install.sh service
...
```bash
tar -xzf wepush-next-0.1.0-alpha.1.tar.gz
sudo ./wepush-next-0.1.0-alpha.1/install/macos/install.sh service
...
```powershell
Expand-Archive .\wepush-next-0.1.0-alpha.1.zip .\release
Set-ExecutionPolicy -Scope Process Bypass
& .\release\wepush-next-0.1.0-alpha.1\install\windows\install.ps1 -Component service
...
- [本版本 Release Notes](releases/0.1.0-alpha.1.md)
- [安全策略](../SECURITY.md)
