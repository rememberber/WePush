...
本文是 `0.1.x` 基线的可执行部署说明。Standalone 使用 SQLite 和本地 Artifact；Server 使用 PostgreSQL、S3-compatible Artifact、两个以上 Service 实例以及外部负载均衡器。Classic 不参与 Next 的构建、安装或运行。
...
- `distribution/target/wepush-next-0.1.0-SNAPSHOT.tar.gz`
- `distribution/target/wepush-next-0.1.0-SNAPSHOT.zip`
- 对应 `.sha256` 文件
...
Desktop 打包器只使用已安装的 Electron，不引入带 Git 构建依赖的额外打包链；macOS、Windows、Linux 必须分别在目标操作系统打包并完成签名/公证流程。
pnpm 已显式信任固定版本 Electron 的原生运行时下载脚本；无法访问 GitHub Release Asset 的网络可在安装依赖时设置受信 `ELECTRON_MIRROR`。macOS 本地开发包自动做 ad-hoc 签名；正式发布设置 `WEPUSH_CODESIGN_IDENTITY` 使用 Developer ID 和 Hardened Runtime，随后仍须在发布流水线完成 Apple Notarization。Windows 正式发行同样应在发布流水线执行 Authenticode 签名。
...
```bash
sha256sum -c wepush-next-0.1.0-SNAPSHOT.tar.gz.sha256
tar -xzf wepush-next-0.1.0-SNAPSHOT.tar.gz
sudo ./wepush-next-0.1.0-SNAPSHOT/install/linux/install.sh all
```
...
```bash
sudo ./wepush-next-0.1.0-SNAPSHOT/install/macos/install.sh all
...
```powershell
Expand-Archive .\wepush-next-0.1.0-SNAPSHOT.zip .\release
Set-ExecutionPolicy -Scope Process Bypass
& .\release\wepush-next-0.1.0-SNAPSHOT\install\windows\install.ps1 -Component all
