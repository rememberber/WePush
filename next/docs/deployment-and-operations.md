...
- `distribution/target/wepush-next-0.1.0-alpha.1.tar.gz`
- `distribution/target/wepush-next-0.1.0-alpha.1.zip`
- 对应 `.sha256` 文件
...
pnpm 已显式信任固定版本 Electron 的原生运行时下载脚本；无法访问 GitHub Release Asset 的网络可在安装依赖时设置受信 `ELECTRON_MIRROR`。`0.1.0-alpha.1` 公开预览版有意不使用商业签名：macOS 只做 ad-hoc 签名且不提交 Apple Notarization，Windows 不做 Authenticode 签名。系统可能显示未知开发者警告，请只从 GitHub Releases 下载并验证 `SHA256SUMS`。未来稳定发行仍应接入 Developer ID/Notarization 与 Authenticode。
...
```bash
sha256sum -c wepush-next-0.1.0-alpha.1.tar.gz.sha256
tar -xzf wepush-next-0.1.0-alpha.1.tar.gz
sudo ./wepush-next-0.1.0-alpha.1/install/linux/install.sh all
```
...
```bash
sudo ./wepush-next-0.1.0-alpha.1/install/macos/install.sh all
...
& .\release\wepush-next-0.1.0-alpha.1\install\windows\install.ps1 -Component all
