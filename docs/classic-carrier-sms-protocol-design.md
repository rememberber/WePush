...
Classic 版新增统一的“运营商协议短信”消息类型，首期支持 CMPP 2.0/3.0、SMGP 3.0、SGIP 1.2 和 SMPP 3.4 的 MT 短信提交。这四种协议均为长连接二进制协议，不应继续套用现有 HTTP 短信供应商的“每条消息创建客户端”模型。

结论：在 Classic 现有账号、消息和任务模型上可实现，首期不需要修改数据库表结构。账号参数和消息内容继续分别保存于 `TAccount.accountConfig` 和 `TMsg.content` JSON 中。
...
- 模板变量替换、测试发送、批量任务、无限推送和模拟发送与现有 Classic 能力保持一致。
- 账号删除、重新配置或应用退出时关闭对应连接。

首期不包含：

- 状态报告（DLR）的落库和发送历史回填。`SUBMIT_RESP` 成功只表示网关已受理，不代表终端已收到。
- MO 上行短信业务处理。
- SGIP 的反向 TCP 监听服务。SGIP 网关通常通过反向连接推送 DLR/MO，这需要可达的本地 IP/端口，应在第二期单独设计。
...
- Java 21 下编译、建连、发送和关闭均无需额外 JVM `--add-opens`。首期无用的 FST/持久化路径从运行时依赖中排除。
- 检查运行时依赖和安装包体积；对文本短信无用的 WAP/MMS/持久化依赖仅在确认不会触发类加载后才能排除。
- 连接不可达和应答丢失时，WePush 设置的超时必须真正结束当次等待。
- 最终分发前对全部运行时依赖做许可证和安全审计。文本编码必需的 `smsj` 声明为 MPL-1.1；WAP/MMS 才使用的 GPL-2.0 `wbxml-stream` 已从 Classic 运行时依赖排除。Netty 保持 4.1 二进制兼容线并由根 POM 统一锁定安全修复版本，不使用协议库原始锁定的 4.1.86.Final。
...
| `registeredDelivery` | 是否请求 DLR | 全部 |
...
首期消息 JSON 只保存 `content` 文本。内容支持 Classic 现有 Velocity 变量。号码仍来自人员表第一列，发送前去除首尾空格，不擅自改写国家码或号段。
...
- 对连接失败由客户端进行有界重连；不在 `MsgSender` 上层对结果不明的提交自动重试。
...
- 消息类型页新增“运营商协议短信”。
- 账号页选择协议后显示通用字段和协议专属字段，切换协议不丢失已输入的其他字段。
...
- 四协议账号配置的 JSON 序列化/反序列化、校验和连接 URI 生成。
- 四协议提交请求字段映射。
...
第二期需新增每次提交记录，持久化协议消息 ID、接收人、提交时间、提交状态、最终状态和 DLR 时间，然后实现 CMPP/SMGP/SMPP 同连接 DLR/MO 及 SGIP 反向监听。最终状态与 Classic 当前的“发送成功”统计必须分开，避免混淆“网关受理”和“终端送达”。
